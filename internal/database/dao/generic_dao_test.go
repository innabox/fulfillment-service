	...
	Describe("Behaviour", func() {
		...
		Describe("Filtering", func() {
			...
			It("Filters results based on tenant visibility", func() {
				// Create ownership logic for the restricted DAO
				restrictedOwnershipLogic := auth.NewMockOwnershipLogic(ctrl)
				restrictedOwnershipLogic.EXPECT().DetermineAssignedOwners(gomock.Any()).
					Return([]string{"test_user"}, nil).
					AnyTimes()
				...
				restrictedTenancyLogic.EXPECT().DetermineVisibleTenants(gomock.Any()).
					Return([]string{"tenant_a"}, nil). // Can only see tenant_a
					AnyTimes()
				...
				restrictedDAO, err := NewGenericDAO[*testsv1.Object]().
					...
					SetDefaultLimit(defaultLimit).
					SetMaxLimit(maxLimit).
					SetOwnershipLogic(restrictedOwnershipLogic).
				...
				_, err = tx.Exec(ctx, `
					insert into objects (id, finalizers, owners, tenants, data)
