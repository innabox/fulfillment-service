...
var _ = Describe("Multitenancy authentication error handling", Label("multitenancy", "autherrors"), func() {
	DescribeTable("returns error when authenticating with invalid token", func(testUrl string) {
	...
	DescribeTable("returns error when user is not authenticated", func(testUrl string) {
		var testUser string
